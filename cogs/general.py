...
class General:
	...
	@commands.command()
	async def choisir(self, *choices):
		...
		if len(choices) < 2:
			await self.bot.say('Pas assez de faces.')
		else:
			await self.bot.say(randchoice(choices))

	@commands.command(pass_context=True)
	async def dé(self, ctx, number : int = 100):
		...
		if number > 1:
			...
			return await self.bot.say("{} Peut-être plus que 1? ;P".format(author.mention))

	@commands.command(pass_context=True)
	async def pièce(self, ctx, user : discord.Member=None):
	...
	async def chrono(self, ctx):
		...
		if not author.id in self.stopwatches:
			...
			await self.bot.say(author.mention + " C'est parti!")
		else:
			tmp = abs(self.stopwatches[author.id] - int(time.perf_counter()))
			tmp = str(datetime.timedelta(seconds=tmp))
			await self.bot.say(author.mention + " Stop! Temps: **" + str(tmp) + "**")
	...
	async def calin(self, user : discord.Member, intensity : int=1):
	...
	@commands.command(no_pm=True, hidden=True)
	async def punch(self, user : discord.Member, intensity : int=1):
		"""Because everyone likes hugs
